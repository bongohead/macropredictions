...
get_treasury_yields = function() {
	...
	urls = c(
		...
		paste0(
			...
			'/all?type=daily_treasury_yield_curve&field_tdr_date_value=',2021:year(today('US/Eastern')),'&page&_format=csv'
		)
	)
...
#' Get TIPS yield data from US Treasury
#'
#' @description
#' Gets yields data from the US Treasury website. Dates included are 2001 - today, subject to a possible 3:30PM lag.
#'
...
#'
#' @examples \dontrun{
#'  get_real_treasury_yields()
#' }
#'
#' @export
get_real_treasury_yields = function() {

	urls = c(
		'https://home.treasury.gov/system/files/276/par-real-yield-curve-rates-2003-2010.csv',
		'https://home.treasury.gov/system/files/276/par-real-yield-curve-rates-2011-2020.csv',
		paste0(
			'https://home.treasury.gov/resource-center/data-chart-center/interest-rates/daily-treasury-rates.csv/',
			2021:year(today('US/Eastern')),
			'/all?type=daily_treasury_real_yield_curve&field_tdr_date_value=',2021:year(today('US/Eastern')),'&page&_format=csv'
		)
	)
	...
	csv %>%
		...
		transmute(
			.,
			varname = paste0('rt', str_pad(ttm_1, 2, pad = '0'), ifelse(ttm_2 == 'Mo', 'm', 'y')),
			ttm = ifelse(ttm_2 == 'Mo', as.numeric(ttm_1), as.numeric(ttm_1) * 12),
			date = mdy(Date),
			value
		) %>%
