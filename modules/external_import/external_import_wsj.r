...
local({
	...
	old_data_path = file.path(Sys.getenv('MP_DIR'), 'modules', 'external_import', 'old_wsj_data', 'forecast_data')

	old_data = map(fs::dir_ls(old_data_path), function(path) {

		vdate = str_extract(fs::path_ext_remove(basename(path)), '(?<=_).*')

		col_names =
			read_csv(path, col_names = F, name_repair = 'universal', show_col_types = F, n_max = 2) %>%
			replace(., is.na(.), '') %>%
			{paste0(.[1,], '-', .[2,])}

		if (length(unique(col_names)) != length(col_names)) stop('WSJ Input Error!')

		read_csv(path, col_names = col_names, skip = 2, show_col_types = F) %>%
...
local({
	...
	old_recess_data = read_csv(
		file.path(Sys.getenv('MP_DIR'), 'modules', 'external_import', 'old_wsj_data', 'recession_data', 'clean_recess.csv'),
		col_types = 'ccccDDd'
	)
