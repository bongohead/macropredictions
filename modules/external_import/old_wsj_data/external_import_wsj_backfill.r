...
#'  These are joined on recession data and the results are dumped in a CSV for use in external_import_wsj.
...
vintage_date_map =
	...
	read_file(
		file.path(Sys.getenv('MP_DIR'), 'modules', 'external_import', 'old_wsj_data', 'recession_data', 'raw_page_html.html')
		) %>%
	read_html() %>%
	html_nodes(., 'ul > li') %>%
...
res =
	read_csv(
		file.path(Sys.getenv('MP_DIR'), 'modules', 'external_import', 'old_wsj_data', 'recession_data', 'raw_recess.csv')
		) %>%
