...
local({
	...
	# Convert CPI into most recent YoY % change by each CPI release date
	# These values will be used for the "1" ttm at each vdate
	last_values_cpi =
		cpi %>%
		slice_max(., date, by = vdate) %>%
		mutate(., date_1y_lag = add_with_rollback(date, months(-12))) %>%
		left_join(
			.,
			transmute(cpi, date_1y_lag = date, lag_vdate = vdate, lag_value = value),
			join_by(date_1y_lag, closest(vdate >= lag_vdate))
		) %>%
		na.omit() %>%
		mutate(., inf = 100 * (value/lag_value - 1)) %>%
		transmute(., vdate, date, value) %>%
		arrange(., date)

	# Now get last values of TIPS & last value of CPI available for each forecast vdate
	tips_available_data =
		expand_grid(vdate = backtest_vdates, ttm = c(1, unique(tips$ttm))) %>%
		left_join(
			.,
			bind_rows(
				last_values_cpi %>% transmute(., h_vdate = vdate, h_date = date, ttm = 1, value),
				tips %>% transmute(., h_vdate = vdate, h_date = date, ttm, value)
			),
...
local({
	...
	fill_outputs =
		...
		map(., function(x) {
			...
			tibble(vdate = x$vdate, date = x$forecast_dates, value = inf)
		}) %>%
		list_rbind()
	...
	forecasts_by_vdate_plot =
		fill_outputs %>%
		filter(., date %in% floor_date(today() - months(0:6), 'month')) %>%
		filter(., vdate >= today() - months(24)) %>%
	...
	forward_plot =
		fill_outputs %>%
		filter(., vdate %in% c(sample(unique(.$vdate), 12), max(vdate))) %>%
