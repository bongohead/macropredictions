...
vintage_date_map =
	...
	read_file(
		file.path(Sys.getenv('MP_DIR'), 'modules', 'external-import', 'old-wsj-data', 'recession-data', 'raw-page-html.html')
		) %>%
	read_html() %>%
	html_nodes(., 'ul > li') %>%
...
res =
	read_csv(
		file.path(Sys.getenv('MP_DIR'), 'modules', 'external-import', 'old-wsj-data', 'recession-data', 'raw-recess.csv')
		) %>%
