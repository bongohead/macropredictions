...
local({
	...
	old_data_path = file.path(Sys.getenv('MP_DIR'), 'modules', 'external-import', 'old-wsj-data', 'forecast-data')
...
local({
	...
	old_recess_data = read_csv(
		file.path(Sys.getenv('MP_DIR'), 'modules', 'external-import', 'old-wsj-data', 'recession-data', 'clean-recess.csv'),
		col_types = 'ccccDDd'
	)
